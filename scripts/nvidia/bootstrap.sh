...
if [ ! -x "${MINICONDA_ROOT}/bin/conda" ] && ! command -v conda >/dev/null 2>&1; then
    echo "[INFO] Installing Miniconda under ${MINICONDA_ROOT}"
...
MODEL_ROOT="${MODEL_ROOT}" bash "${PROJECT_ROOT}/scripts/setup/setup_env_nvidia.sh"
